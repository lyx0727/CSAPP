...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)){
        ...
        /* block SIGCHILD to avoid "deletejob" before "addjob" */
        sigprocmask(SIG_BLOCK, &mask_child, &mask_prev);

        if((pid = Fork()) == 0){    /* child */
            /* child inherit parent's mask */
...
int builtin_cmd(char **argv) 
{
    ...
    if(!strcmp(argv[0], "quit"))    /* quit command */
        ...
        /* visit global variable, block all */
        sigfillset(&mask_all);
...
void do_bgfg(char **argv) 
{
    ...
    if(!strcmp(argv[0], "bg")){
        ...
        sigprocmask(SIG_SETMASK, &mask_prev, NULL);
		Kill(-pid, SIGCONT); 
		waitfg(pid);    /* wait for the bg child end */
		return;
    }
